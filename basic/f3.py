'''
    클라이언트가 서버에게 데이터를 보내는 방법 
        - 방법 : method:GET,POST.,PUT...
            < - http(or https)프로토콜(통신 규약,서로 데이터를 주고받는 약속)에서 정의
            http는 헤더(고정크기)를 먼저 전송, 바디(가변크기)를 나중에 전송
            get방식은 헤더만 전송 -> 고정 크기만 전송 => 보낼량의 최대치가 정해져있다 => 소량만 전송
                http헤더만 알면, 패킷(전송 데이터를)가로 채서 데이터를 해킹할 수 있다 => 보안 취약
            post 방식은 헤더(바디의 크기가 세팅되어 있음) 전송 -> 바디 전송 -> 대량 전송 가능,구조를 모르기 때문에 보안에
            상대적으로 우수,암호화 가능 -> 보안 양호 (상대적)

        - 방식
            - form 전송 : <form>태그 사용, 화면이 껌벅(순식간에 진행, X로 주소창앞에 아이콘 변경)
...
# #,<,>는 전달 안됨, 한글,특수문자,영대,영소 가능
# url인코딩 으로 전달하면 모두 전달 가능
