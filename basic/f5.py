...
app = Flask(__name__)
# 세션을 위해서 시크릿 키 지정
app.secret_key = 'secret_key' #임의 값, 통상 해시값 활용

# 로그인을 하여 세션을 얻은 후 홈페이지를 진행해야 사이트의 내용을 보여주겠다 => 컨셉
@app.route('/')
def home():
    if not 'uid' in session:
        #return redirect('/login') #url을 사용할 때는  하드코딩하지않는다
...
def login():
    ...
    if request.method == 'GET':
        ...
        if result:# 3. 회원이면
            ...
            #       단점: 접속 유저가 많으면 서버측 메모리에 부하가 온다 -> 대체제가 필요/대안필요
            #       JWT를 사용하여 보완(사이트 구성시 인증쪽에서 활용: 차주 진행)
